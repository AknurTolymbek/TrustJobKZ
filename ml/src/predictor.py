...
import re, json, warnings, joblib, os
...
_MODEL_TEXT = joblib.load(
    os.path.join(MODELS_DIR, "model_text_only.joblib")
)

_MODEL_HYB = joblib.load(
    os.path.join(MODELS_DIR, "model_hybrid.joblib")
)

_TFIDF = joblib.load(
    os.path.join(ARTIFACTS_DIR, "tfidf_vectorizer.joblib")
)

_SCALER = joblib.load(
    os.path.join(ARTIFACTS_DIR, "numeric_scaler.joblib")
)

with open(
    os.path.join(ARTIFACTS_DIR, "model_config.json")
) as f:
    _CFG = json.load(f)
...
with open(
    os.path.join(ARTIFACTS_DIR, "suspicious_keywords.json"),
    encoding="utf-8"
) as f:
    _KWD = json.load(f)
...
with open(
    os.path.join(ARTIFACTS_DIR, "kz_column_map.json"),
    encoding="utf-8"
) as f:
    KZ_MAP = json.load(f)
